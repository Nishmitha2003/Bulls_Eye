...
int shoot=0,bang=0;
int counter1=0,counter2=0,counter3=0,count=0;
int x,y;
int game,instruct;
char tmp_str[40];
void display2();
...
void instructions()
{
	...
	draw_instruct("1. Right click on mouse",300,1700);
	draw_instruct("  to play",300,1500);
	draw_instruct("2. Press f to shoot",300,1300);
	draw_instruct("3. Red ball is the target",300,1100);
	glFlush();
}
...
void display3()
{
	...
	glColor3f(1, 1, 0);
	glRasterPos2f(-0.2, 0.9);
	sprintf(tmp_str, "Score: %d", counter3);
	Write(tmp_str);
	glPopMatrix();
	glPushMatrix();
	glColor3f(1.0,0.0,0.0);
...
int main(int argc,char **argv)
{
	...
	glutInitWindowPosition(1200,900);
	...
	game=glutCreateWindow("Bulsseye");
	init();
